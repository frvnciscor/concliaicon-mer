"""
App Streamlit - Conciliación Minas El Romeral (MER) v7
Mejoras sobre v6:
  NEW 1. Tab 4 — tabla resumen kt por paso de cascada + descarga CSV
  NEW 2. Tab 6 — precisión, recall, F1, dilución, pérdida (matriz binaria)
  NEW 3. Tab 6 — opción acumulado (meses 1..sel) para matriz binaria
"""
...
with st.sidebar:
    ...
    col_lp = st.color_picker("LP", "#4472C4")
    col_mp = st.color_picker("MP", "#ED7D31")
    col_cp = st.color_picker("CP", "#70AD47")
    st.markdown("---")
...
with tab2:
    ...
    n_meses  = len(tabla_plot)
    nan_ser  = pd.Series([np.nan] * n_meses)
    ton_mod  = tabla_plot[col_ton_m]  if col_ton_m  in tabla_plot.columns else nan_ser
    cal_mod  = tabla_plot[col_cal_m]  if col_cal_m  in tabla_plot.columns else nan_ser
    ton_cp   = tabla_plot['tonelaje_cp'] if 'tonelaje_cp' in tabla_plot.columns else nan_ser
    cal_cp   = tabla_plot[col_cal_cp] if col_cal_cp in tabla_plot.columns else nan_ser

    fig, ax1 = make_fig((14, 5))
    pos = np.arange(n_meses); bw = 0.4
    ...
    if show_annot:
        annotate_bars(ax1, b1, cal_mod)
        annotate_bars(ax1, b2, cal_cp)

    ax1.set_ylabel('Tonelaje (kt)', color='#444')
    ax1.set_xticks(pos); ax1.set_xticklabels(tabla_plot['mes'], color='#444')
    ...
    ax2.plot(pos, cal_mod, marker='.', color=bar_color, lw=1.8, label=f'{var_cal.upper()} {label_mod}')
    ax2.plot(pos, cal_cp,  marker='.', color=col_cp,   lw=1.8, label=f'{var_cal.upper()} CP')
    ...
    for cc_col, label, color in [('cal_lp',f'{var_cal.upper()} LP',col_lp),
                                  ('cal_mp',f'{var_cal.upper()} MP',col_mp),
                                  ('cal_cp',f'{var_cal.upper()} CP',col_cp)]:
        if cc_col in tabla_trim.columns:
            ax2b.plot(pos2, tabla_trim[cc_col], marker='.', color=color, lw=1.8, label=label)
...
with tab6:
    ...
    if df_mp.empty:
        ...
        try:
            ...
            ax.set_facecolor('white')
            sns.heatmap(ct_pct, cmap='YlOrBr', annot=annot, fmt='',
