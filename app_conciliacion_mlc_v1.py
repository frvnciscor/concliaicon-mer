...
with tab3:
    ...
    TAM=12.5
    BENCH=15.0  # altura de banco MLC: dim_z=15m, saltos de 15m
    try:
        z_col='centroid_z' if 'centroid_z' in df.columns else 'centroid_z_mp'
        # Redondear cotas al múltiplo de 15m más cercano para evitar ruido de flotante
        z_raw = df.loc[df['periodo']==mes, z_col].dropna()
        z_bancos = sorted(set((z_raw / BENCH).round().astype(int) * BENCH))
    except: z_bancos=[]
    ...
    if not z_bancos:
        ...
        # Banco 0 = cota mínima, +1 = +15m, +2 = +30m
        cota=z_bancos[bidx]; banco_real=cota-BENCH/2
        ...
        try:
            # Filtrar exactamente un banco: centroide ±7.4m (estricto < 7.5)
            df_mp_c=df[abs(df.get('centroid_z_mp',df.get('centroid_z',pd.Series()))-cota)<7.5].copy()
            df_mp_c=(df_mp_c.sort_values('periodo',ascending=False)
                     .drop_duplicates(subset=['centroid_x_mp','centroid_y_mp'],keep='first'))
            df_cp_c=df[abs(df.get('centroid_z_cp',df.get('centroid_z',pd.Series()))-cota)<7.5].copy()
...
with tab5:
    ...
    with _d3:
        ...
        ue_vals = sorted([int(x) for x in df[col_ue].dropna().unique()
                          ...
                          and 1 <= float(x) <= 6])
        ue_vals = ue_vals or [1, 2, 3, 4, 5, 6]
