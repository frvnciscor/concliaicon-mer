...
def clasificar(df, criterios):
    ...
    Opera directamente sobre el df sin copiar — ahorra memoria.
    """
...
cache_key = (
    tuple((name, len(content)) for name, content in lp_b),
    tuple((name, len(content)) for name, content in mp_b),
    tuple((name, len(content)) for name, content in cp_b),
)
...
# Clasificar ore en tiempo real sobre df_raw directamente (sin copy)
df = clasificar(df_raw, criterios)
del df_raw  # liberar referencia — df_raw sigue en cache de Streamlit pero no en memoria activa
...
def build_conciliaciones(df):
    try:
        ...
        return conc_lp, conc_cp_v2, conc_mp_all
    except Exception:
        empty = pd.DataFrame()
        return empty, empty, empty

# ─── CONCILIACIONES (Cell 30) ───
conc_lp, conc_cp_v2, conc_mp_all = build_conciliaciones(df)
...
with tab1:
    ...
    # ── Desviaciones ──
    # Budget   = (CP - LP) / LP  — LP en MLC: tonelaje, fe, fem, dtt (sin sufijo)
    # Forecast = (CP - MP) / MP  — MP en MLC: tonelaje_mp, fe_mp, fem_mp, dtt_mp
    st.markdown("---")
    ...
    st.markdown("---")
    cols_d=[c for c in ['mes','fe','fem','fedtt','dtt','p','s','volumen','tonelaje','fino',
                        'fe_mp','fem_mp','fedtt_mp','dtt_mp','p_mp','s_mp','volumen_mp','tonelaje_mp','fino_mp',
                        'fe_cp','fem_cp','fedtt_cp','dtt_cp','p_cp','s_cp','volumen_cp','tonelaje_cp','fino_cp']
...
with tab3:
    ...
    st.markdown("*Bloques 12.5×12.5×15 m — MLC*")
    ...
    TAM=12.5
    BENCH=15.0  # altura de banco MLC: dim_z=15m
    try:
        # Cell 27: cota = df.loc[df['periodo']==mes, 'centroid_z'].min()
        # centroid_z es LP sin sufijo — igual que el notebook
        z_bancos = sorted(df.loc[df['periodo']==mes, 'centroid_z'].dropna().unique())
    except: z_bancos=[]

    if not z_bancos:
        st.warning(f"Sin bloques para periodo {mes}.")
    else:
        n_b=min(3,len(z_bancos))
        nb1,nb2,_=st.columns([1,1,3])
        with nb1: bidx=st.number_input("Banco (0=más bajo, +1, +2)",min_value=0,max_value=n_b-1,value=0,step=1,key="bidx")
        # banco_real = cota - 7.5 (igual que notebook)
        cota=z_bancos[bidx]; banco_real=cota-7.5
        with nb2: st.markdown(f"<div class='cutoff-box'>Banco<br><b>Z={banco_real:.1f}m</b><br>(centroide Z={cota:.1f}m)</div>",unsafe_allow_html=True)
        try:
            # Cell 27: igualdad exacta sobre centroid_z_mp y centroid_z_cp
            df_mp_c=df[df['centroid_z_mp']==cota].copy()
            df_mp_c=(df_mp_c.sort_values('periodo',ascending=False)
                     .drop_duplicates(subset=['centroid_x_mp','centroid_y_mp'],keep='first'))
            df_cp_c=df[df['centroid_z_cp']==cota].copy()
            ...
            def dibujar_bloques_vec(ax, df_i, col_clase, cx, cy, color_map):
                """Dibuja bloques con PatchCollection — sin iterrows."""
                if df_i.empty: return
                xs      = df_i[cx].values
                ys      = df_i[cy].values
                clases  = df_i[col_clase].astype(str).str.lower().values if col_clase in df_i.columns else ['esteril']*len(df_i)
                periodos= df_i['periodo'].values
                rects   = [Rectangle((x-TAM/2, y-TAM/2), TAM, TAM) for x,y in zip(xs,ys)]
                fcolors = [(*color_map.get(k,(0.8,0.8,0.8))[:3], 1.0 if p==mes else 0.25)
                           for k,p in zip(clases,periodos)]
                ax.add_collection(PatchCollection(rects, facecolors=fcolors, edgecolors='none'))
                ax.autoscale()
            ...
            if tipo_m=="Clasificación (Ore)":
                ...
                for ax,(df_i,lbl,cx,cy) in zip(axs,pnl):
                    dibujar_bloques_vec(ax, df_i, ore_c[lbl], cx, cy, clrs)
                    ax.set_aspect('equal'); sax(ax,f'{lbl} — Banco={banco_real:.1f}m')
                ...
                for ax,(df_i,lbl,cx,cy) in zip(axs,pnl):
                    dibujar_bloques_vec(ax, df_i, occ_c.get(lbl,'ocurrencia_mp'), cx, cy, co)
                    ax.set_aspect('equal'); sax(ax,f'{lbl} — Banco={banco_real:.1f}m')
...
with tab4:
    ...
    _cs0,_=st.columns([1,3])
    with _cs0: modelo_casc_mes=st.radio("Modelo vs CP:",["MP","LP"],horizontal=True,key="r_casc_mes_mlc")
    cy0=st.number_input("Ton. mín (kt)",value=0,step=50,key="cmy0")
    cy1=st.number_input("Ton. máx (kt)",value=2000,step=50,key="cmy1")
    try:
    ...
    _cas0,_=st.columns([1,3])
    with _cas0: modelo_casc_acum=st.radio("Modelo vs CP:",["MP","LP"],horizontal=True,key="r_casc_acum_mlc")
    cay0=st.number_input("Ton. mín (kt)",value=0,step=100,key="cay0")
    cay1=st.number_input("Ton. máx (kt)",value=15000,step=500,key="cay1")
    try:
...
with tab6:
    ...
    _cq2,_cq3=st.columns(2)
    cutoff_q=st.number_input("Corte FeM (%)",value=float(cutoff_fem),step=0.5,key="cq")
