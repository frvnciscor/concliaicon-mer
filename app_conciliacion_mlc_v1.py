...
with tab3:
    ...
    if not z_bancos:
        ...
        try:
            # Filtrar con tolerancia ±1m para absorber ruido de flotante
            df_mp_c=df[abs(df.get('centroid_z_mp',df.get('centroid_z',pd.Series()))-cota)<=1.0].copy()
            df_mp_c=(df_mp_c.sort_values('periodo',ascending=False)
                     .drop_duplicates(subset=['centroid_x_mp','centroid_y_mp'],keep='first'))
            df_cp_c=df[abs(df.get('centroid_z_cp',df.get('centroid_z',pd.Series()))-cota)<=1.0].copy()
