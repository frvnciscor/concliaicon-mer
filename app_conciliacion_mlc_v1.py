...
with tab2:
    ...
    b1=ax1.bar(pos-bw/2,tm, bw,color=bclr,alpha=0.85,label=f'Ton {mod_sel}')
    b2=ax1.bar(pos+bw/2,tcp,bw,color=col_cp,alpha=0.85,label='Ton CP')
    ...
    for tc,cc_c,lbl,clr,idx in [('ton_lp','cal_lp','LP',col_lp,0),('ton_mp','cal_mp','MP',col_mp,1),('ton_cp','cal_cp','CP',col_cp,2)]:
        if tc in tt.columns:
            bars=ax1.bar(p2+(idx-1)*bw2,tt[tc],bw2,color=clr,alpha=0.85,label=f'Ton {lbl}')
