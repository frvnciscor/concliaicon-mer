"""
App Streamlit - Conciliación Pleito Central (PLC) v1
Basada en la arquitectura de app_conciliacion_mer_v9.py
Particularidades PLC vs MER:
  - Ley de corte: fe >= 27 AND fem >= 25 (sin mag)
  - Marginal: ue_fe >= 1 AND ue_fe < 7 con fe o fem bajo corte
  - Ocurrencias: ['bre', 'dis', 'est'] (sin mac, gyd)
  - Densidad default: -99 → 2.7
  - p y s: -99 → NaN (limpieza especial)
  - Tab extra: Gráfico cuadrantes FeM (exclusivo PLC)
"""
...
def clasificar_ore_plc(row, criterios, sufijo=''):
    """
    Clasificación PLC:
    - mineral:  fe >= 27 AND fem >= 25
    - marginal: ue_fe >= 1 AND ue_fe < 7 AND (fe < 27 OR fem < 25)
    - esteril:  resto
    Criterios adicionales del sidebar se aplican encima de este esquema base.
    """
    ue_val = row.get(f'ue_fe{sufijo}', 0)
    if pd.isna(ue_val):
        ue_val = 0

    # Evaluar criterios del sidebar
    cumple = True
    for c in criterios:
...
# ─────────────────────────────────────────────
with st.sidebar:
    st.markdown("## ⛏️ PLC · Conciliación")
    st.markdown("---")
    ...
    st.markdown("<div class='cutoff-box'>Criterios para mineral/marginal/estéril.<br>"
                "PLC base: fe ≥ 27 AND fem ≥ 25</div>",
                unsafe_allow_html=True)
    n_criterios = st.number_input("Número de criterios", min_value=1, max_value=3, value=2, step=1)
    # Defaults PLC: fe >= 27, fem >= 25
    DEFAULTS = [{'var':'fe','op':'>=','val':27.0},
                {'var':'fem','op':'>=','val':25.0},
    ...
    st.markdown(f"<div class='cutoff-box'>🎯 {resumen_corte}<br>"
                f"Marginal: ue_fe ∈ [1,7)</div>",
                unsafe_allow_html=True)
    st.markdown("---")
...
st.markdown("# Conciliación Mensual · Pleito Central")
...
def build_tabla(df, df_mp):
    def agg_model(df_in, ore_col, ton_col, suf):
        return (
            ...
            .apply(lambda x: pd.Series({
                ...
                f'volumen{suf}':   x['proportional_volume'].sum() if 'proportional_volume' in x.columns else np.nan,
    ...
    if not df_mp.empty:
        t3 = (
            ...
            .apply(lambda x: (lambda f: pd.Series({
                ...
                'volumen_mp':   f['proportional_volume'].sum() if 'proportional_volume' in f.columns else np.nan,
    ...
    for suf in ['', '_mp', '_cp']:
        ...
        for col in ['volumen', 'tonelaje', 'fino']:
            cn = f'{col}{suf}'
...
# Garantizar tabla_plot con SIEMPRE 12 meses
...
with tab1:
    ...
    st.markdown(f"*Mineral: {cutoff_str} · Marginal: ue_fe ∈ [1,7)*")
    ...
    # ── Desviaciones ──
    # Budget   = (CP - LP) / LP → referencia = LP
    # Forecast = (CP - MP) / MP → referencia = MP
    st.markdown("#### Desviaciones")

    def _pct(src, col_ref, col_cp):
        try:
            ref = src[col_ref].values[0]; cp = src[col_cp].values[0]
            if pd.isna(ref) or pd.isna(cp) or ref == 0: return None
            return (float(cp) - float(ref)) / abs(float(ref)) * 100
        except: return None
    ...
    def _fmt_val(v, fmt="{:,.1f}"):
        if v is None: return "—"
        return fmt.format(v)

    VARS_DEV = [
        ("Fe (%)",        "fe",       "fe_mp",       "fe_cp"),
        ("FeM (%)",       "fem",      "fem_mp",      "fem_cp"),
        ("FeDTT (%)",     "fedtt",    "fedtt_mp",    "fedtt_cp"),
        ("Tonelaje (kt)", "tonelaje", "tonelaje_mp", "tonelaje_cp"),
    ]

    def render_reporte(src, titulo):
        st.markdown(f"**{titulo}**")
        cols = st.columns(len(VARS_DEV))
        for w, (lbl, col_lp, col_mp, col_cp) in zip(cols, VARS_DEV):
            cp_v   = _val(src, col_cp)
            bud_p  = _pct(src, col_lp, col_cp)
            fore_p = _pct(src, col_mp, col_cp)
            bud_s,  bud_cls  = _fmt_pct(bud_p)
            fore_s, fore_cls = _fmt_pct(fore_p)
            with w:
                st.markdown(f"""<div class='metric-card'>
                    <div class='metric-label'>{lbl}</div>
                    <div class='metric-value'>CP: {_fmt_val(cp_v)}</div>
                    <div class='metric-dev {bud_cls}' style='font-size:11px;'>Budget: {bud_s}</div>
                    <div class='metric-dev {fore_cls}' style='font-size:11px;'>Forecast: {fore_s}</div>
                </div>""", unsafe_allow_html=True)

    render_reporte(mes_row,   f"Mes — {mes_abr}")
    st.markdown("")
    render_reporte(total_row, "Acumulado anual")

    st.markdown("---")

    cols_d = [c for c in [
        'mes', 'fe', 'fem', 'fedtt', 'p', 's', 'volumen', 'tonelaje', 'fino',
        'fe_mp', 'fem_mp', 'fedtt_mp', 'p_mp', 's_mp', 'volumen_mp', 'tonelaje_mp', 'fino_mp',
        'fe_cp', 'fem_cp', 'fedtt_cp', 'p_cp', 's_cp', 'volumen_cp', 'tonelaje_cp', 'fino_cp'
...
with tab2:
    ...
    bw = 0.4
    b1 = ax1.bar(pos - bw/2, ton_mod, bw, color=bar_color,  label=f'Ton {label_mod}')
    b2 = ax1.bar(pos + bw/2, ton_cp,  bw, color=col_cp,     label='Ton CP')
    if show_annot:
    ...
    ax1.set_title(f'Extracción Mensual {anio} — Pleito Central\n{cutoff_str}',
                  color='#222', fontsize=11, pad=10)
    plt.tight_layout()
    png_mensual = save_png(fig)
    st.pyplot(fig); plt.close()
    st.download_button("⬇️ PNG Mensual", png_mensual,
                       "extraccion_mensual_plc.png", "image/png", key="dl_png_mensual")
    ...
    ax1.set_title(f'Extracción Trimestral {anio} — Pleito Central\n{cutoff_str}',
                  color='#222', fontsize=11, pad=10)
    plt.tight_layout()
    png_trim = save_png(fig2)
    st.pyplot(fig2); plt.close()
    st.download_button("⬇️ PNG Trimestral", png_trim,
                       "extraccion_trimestral_plc.png", "image/png", key="dl_png_trim")
    ...
    ax3.set_title(f'Gráfico Acumulado {anio} — Pleito Central\n{cutoff_str}',
                  color='#222', fontsize=11, pad=10)
    plt.tight_layout()
    png_acum = save_png(fig3)
    st.pyplot(fig3); plt.close()
    st.download_button("⬇️ PNG Acumulado", png_acum,
                       "extraccion_acumulada_plc.png", "image/png", key="dl_png_acum")
    ...
    if df_mp.empty:
        ...
        try:
            ...
            if tipo_mapa == "Clasificación (Ore)":
                ...
                # PLC: bre, dis, est (sin mac, gyd)
                co = {'bre':(0.1,0.3,0.9), 'dis':(1.0,0.9,0.1),
                      'est':(0.75,0.75,0.75)}
            ...
            st.download_button("⬇️ PNG Bloques", png_bloques,
                               "mapa_bloques_plc.png", "image/png", key="dl_png_bloques")
...
with tab4:
    ...
    if df_mp.empty:
        ...
        dfcp_c['ore_cp'] = dfcp_c.apply(lambda r: clasificar_ore_plc(r, criterios, '_cp'), axis=1)
        ...
        fig_c1, df_res_c1 = cascada(df_casc_mes, lbl_proy_mes, f"Real CP {mes_abr}",
                                     f"Conciliación Mensual PLC — {mes_abr} ({modelo_casc_mes} vs CP)",
        ...
        with d_casc_m1:
            st.download_button("⬇️ HTML Cascada Mensual", plotly_to_html(fig_c1),
                               f"cascada_mensual_plc_{mes_abr}.html", "text/html", key="dl_casc_mes")
        with d_casc_m2:
            st.download_button("⬇️ CSV Cascada Mensual", csv_bytes(df_res_c1),
                               f"cascada_mensual_plc_{mes_abr}.csv", "text/csv", key="dl_casc_mes_csv")
        ...
        fig_c2, df_res_c2 = cascada(df_casc_acum, lbl_proy_acum, "Real CP acum.",
                                     f"Conciliación Acumulada PLC — hasta {mes_abr} ({modelo_casc_acum} vs CP)",
        ...
        with d_casc_a1:
            st.download_button("⬇️ HTML Cascada Acumulada", plotly_to_html(fig_c2),
                               f"cascada_acumulada_plc_{mes_abr}.html", "text/html", key="dl_casc_acum")
        with d_casc_a2:
            st.download_button("⬇️ CSV Cascada Acumulada", csv_bytes(df_res_c2),
                               f"cascada_acumulada_plc_{mes_abr}.csv", "text/csv", key="dl_casc_acum_csv")
...
# ─────────────────────────────────────────────
# TAB 6 — CUADRANTES FeM (EXCLUSIVO PLC)
# ─────────────────────────────────────────────
            ...
            if df_fem.empty:
                ...
                with mx_col:
                    ...
                    st.download_button("⬇️ PNG Ore/Waste", png_ow,
                                       f"matriz_ow_plc_{mes_abr}.png", "image/png", key="dl_png_ow")
                ...
                with dl_ow1:
                    st.download_button("⬇️ CSV Ore/Waste (%)", csv_bytes(ct_ow_pct.reset_index()),
                                       f"ow_pct_plc_{mes_abr}.csv", "text/csv", key="dl_ow_csv")
                with dl_ow2:
                    st.download_button("⬇️ CSV Conteos", csv_bytes(ct_raw_ow.reset_index()),
                                       f"ow_raw_plc_{mes_abr}.csv", "text/csv", key="dl_ow_raw")
...
with tab7:
    ...
    if df_mp.empty:
        ...
        try:
            ...
            dmp_occ = dmp_occ.copy(); dmp_occ['ocurrencia'] = dmp_occ['ocurrencia'].replace('nn','est')
            dcp_occ = dcp_occ.copy(); dcp_occ['ocurrencia'] = dcp_occ['ocurrencia'].replace('nn','est')
            ...
            with col_o:
                ...
                st.download_button("⬇️ PNG Ocurrencia", png_occ,
                                   f"matriz_occ_plc_{mes_abr}.png", "image/png", key="dl_png_occ")
            ...
            with dl_o1:
                st.download_button("⬇️ CSV Ocurrencia (%)", csv_bytes(ct_occ_pct.reset_index()),
                                   f"occ_pct_plc_{mes_abr}.csv", "text/csv", key="dl_occ_csv")
            with dl_o2:
                st.download_button("⬇️ CSV Conteos", csv_bytes(ct_raw_occ.reset_index()),
                                   f"occ_raw_plc_{mes_abr}.csv", "text/csv", key="dl_occ_raw")
