...
with tab1:
    ...
    # Tarjeta tipo imagen: Fe, FeM, Tonelaje — CP + Budget% + Forecast%
    VARS_DEV = [
        ("Fe (%)",       "fe",       "fe_mp",       "fe_cp"),
    ...
    def render_reporte(src, titulo):
        ...
        for w, (lbl, col_lp, col_mp, col_cp) in zip(cols, VARS_DEV):
            ...
            bud_p  = _pct(src, col_lp, col_cp)   # (CP-LP)/LP
            fore_p = _pct(src, col_mp, col_cp)    # (CP-MP)/MP
...
with tab2:
    ...
    with cb: ton_ymax_m = st.number_input("Ton. máx (kt)", value=600,  step=50,  key="ty1m")
    ...
    with cd2: ley_ymax_t = st.number_input("Ley máx (%)",  value=40.0, step=1.0, key="ly1t")
    ...
    fig, ax1 = plt.subplots(figsize=(30/2.54, 10/2.54), facecolor='white')
    ax1.set_facecolor('white')
    bw = 0.4
    b1 = ax1.bar(pos - bw/2, ton_mod, bw, color=bar_color,  label=f'Tonelaje {label_mod}')
    b2 = ax1.bar(pos + bw/2, ton_cp,  bw, color=col_cp,     label='Tonelaje CP')
    if show_annot:
    ...
    ax2.plot(pos, cal_mod, marker='.', color=line_color,   lw=1.8, label=f'Fe {label_mod}')
    ax2.plot(pos, cal_cp,  marker='.', color=col_cp_line,  lw=1.8, label='Fe CP')
    ...
    fig2, ax1 = plt.subplots(figsize=(25/2.54, 10/2.54), facecolor='white')
    ax1.set_facecolor('white')
    pos2 = np.arange(len(tabla_trim)); bw2 = 0.3
    for tc, cc_col, label, color, idx in [('ton_lp','cal_lp','LP',col_lp,0),
        ...
        if tc in tabla_trim.columns:
            bars = ax1.bar(pos2+(idx-1)*bw2, tabla_trim[tc], bw2,
                           color=color, label=f'Tonelaje {label}')
            if show_annot:
    ...
    fig3, ax3 = plt.subplots(figsize=(30/2.54, 10/2.54), facecolor='white')
    ...
    if df_mp.empty:
        ...
        try:
            ...
            if df_fem.empty:
                ...
                # Scatter (fila 1) + Matriz (fila 2) — mismo criterio fem >= cutoff
                ...
                with sc_col:
                    ...
                    style_ax(ax)
                    # Etiquetas en centro de cada cuadrante
                    c = cutoff_fem_q
                    hi = (c + max_fem) / 2   # centro eje alto
                    lo = c / 2               # centro eje bajo
